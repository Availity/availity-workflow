...
function possibleNewline(json) {
  const lastChar = (json && json.slice(-1) === '\n') ? '\n' : '';
  return lastChar;
}
...
function space(json) {
  ...
  const result = match[1] ? '\t' : match[2].length;
  return match ? result : '';
}
...
module.exports = function(cli) {
  ...
  _.forEach(cli.manifests, function(data, key) {
    ...
    const payload = _.merge({}, data.json, attrs);
    const raw = JSON.stringify(payload, null, data.raw ? space(data.raw) : 2) + possibleNewline(data.raw);
