...
export default async ({ template: templateUrl, appPath, branchOverride }) => {
  try {
    ...
    if (parsed.protocol && parsed.host) {
      Logger.failed(
        'It looks like you forgot to add a name for your new project. Try running instead "npx @availity/workflow-vite init new-project"'
      );
      return;
    }
  } catch {
  ...
  if (hostedInfo) {
    ...
    Logger.failed('Could not find Hosted Git Info for the Project.');
  }
};
