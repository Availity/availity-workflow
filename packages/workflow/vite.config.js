...
const { toViteProxy } = require('./scripts/proxy');
...
const buildViteConfig = (settings) => {
  ...
  // Polyfill injection (replaces react-app-polyfill/stable and navigator.sendbeacon entry)
  const polyfillPlugin = {
    name: 'availity-polyfills',
    transformIndexHtml(html) {
      // Inject polyfill imports before the app entry script
      return html.replace(
        '<script type="module"',
        `<script type="module">import 'react-app-polyfill/stable';import 'navigator.sendbeacon';</script>\n  <script type="module"`
      );
    }
  };
  plugins.push(polyfillPlugin);
